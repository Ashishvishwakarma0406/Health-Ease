...
const PatientProfilePage = () => {
  ...
  useEffect(() => {
    ...
    const fetchUserAppointments = async () => {
      try {
        const response = await axios.get(
          `${API_BASE_URL}/api/users/appointments/${username}`
        );
    ...
    const fetchVirtualAppointments = async () => {
      try {
        const response = await axios.get(
          `${API_BASE_URL}/api/users/virtual-appointments/${username}`
        );
        setVirtualAppointments(response.data);
      } catch (error) {
